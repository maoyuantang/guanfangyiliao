...
$.ajax({
    url: "/m/v1/api/inquiry/article/getArticleDetails?token="+token,
    type: "GET",
    dataType: "json",
    beforeSend:index,
    ...
    success: function (data) {
        ...
        if(data.errCode==0) {
            ...
            if (navigator.userAgent.match(/(iPhone|iPod|iPad)/i)) {
                window.webkit.messageHandlers.submit.postMessage(data.body.openComment);
                window.webkit.messageHandlers.visible.postMessage(data.body.visible);
...
function editArticleApp(num) {
    $.ajax({
        url: "/api/v1s1/article/editArticleApp",//编辑文章说是不要
        type: "POST",
        dataType: "json",
        // beforeSend:index,
        beforeSend: function (xhr) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.errCode==0) {
                ...
                // appQueryCommentList();
                setTimeout(function () {
                    var t = $('#visible').offset().top;//  获取需要跳转到标签的top值
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.errCode==0) {
                var userId=data.body.userId;
                var pahtml="";
                var img="/api/v1s1/commonly/download?downType=01&secId="+estxt+"&userId="+userId+"&fileId=null&layer=00&imageType=00"?"/api/v1s1/commonly/download?downType=01&secId="+estxt+"&userId="+userId+"&fileId=null&layer=00&imageType=00":"../images/face.png";
                pahtml+='<div class="weui-row weui-back-fff">'
                    +'<div class="weui-col-30">'
                    +'<div class="weui-media-box__hd">'
                    +'<img class="weui-media-box__thumb weui-img-wi" src="'+img+'">'
