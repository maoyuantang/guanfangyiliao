<template>
  <div
    class="family-medicine-management"
  ...
  >
    ...
    <div class="family-medicine-management-body">
      <div
        class="family-medicine-management-body-part-one"
        v-show="barInfo.i===0"
      >
        <div class="family-medicine-management-middle">
          <div class="family-medicine-management-middle-left">
            <selftag
              ...
              @reback="getDepartmentSelect"
            ></selftag>
            <selftag
              v-model="bussModuleList"
              @reback="bussModuleSelect"
            ></selftag>
            <selftag
              v-model="bussTypeList"
              @reback="bussTypeSelect"
            ></selftag>
          </div>
          <div class="family-medicine-management-middle-right">
            <div class="family-medicine-search">
      ...
      <div
        class="family-medicine-management-body-part-two"
        v-show="barInfo.i===1"
      >
        <div class="part-two-head">
          <div class="part-two-head-left">
            <selftag
              v-model="statisticsInfo.departmentList"
              @reback="getStatisticsDepartmentSelect"
            ></selftag>
          </div>
          <div class="part-two-head-right">
            <publicTime @timeValue="timeValueFun"></publicTime>
            <el-select
              v-model="statisticsInfo.countMethod.select.value"
              clearable
              placeholder="请选择"
            >
              <el-option
    ...
    >
      <div class="family-new-alert">
        <!-- 业务类型 -->
        <div
          class="family-new-alert-normal-item"
          v-if="testData.businessTypeList.show"
        >
          ...
          <div class="family-new-alert-normal-item-value">
            <el-select
              v-model="testData.businessTypeList.default.value"
              clearable
              placeholder="请选择"
              size="mini"
              v-if="testData.businessTypeList.default.value!=='customize'"
            >
              <el-option
        ...
        <div
          class="family-new-alert-normal-item"
          v-if="testData.businessTemplate.show"
        >
          <div class="family-new-alert-normal-item-name">
            <span>业务模板:</span>
          </div>
          <div class="family-new-alert-normal-item-value">
            <el-select
              v-model="testData.businessTemplate.default.value"
              clearable
              placeholder="请选择"
              size="mini"
              @change="byStencilModel(testData.businessTemplate.default.value)"
            >
              <el-option
        ...
        <div
          class="family-new-alert-normal-item"
          v-if="testData.businessName.show"
        >
          <div class="family-new-alert-normal-item-name">
            <span>业务名称:</span>
          </div>
        ...
        >
          ...
          <div class="family-new-alert-normal-item-value">
            ...
            >{{unitMap.find(item=>item.en===testData.businessTemplate.default.value)?unitMap.find(item=>item.en===testData.businessTemplate.default.value).unit:''}}</span>
          </div>
        </div>
        ...
        <div
          class="family-new-alert-normal-item"
          v-if="testData.departmentList.show"
        >
          <div class="family-new-alert-normal-item-name">
            <span>科室:</span>
          </div>
          <div class="family-new-alert-normal-item-value">
            <el-select
              v-model="testData.departmentList.default.value"
              clearable
              placeholder="请选择"
              size="mini"
            >
        ...
        >
          ...
          <div class="family-new-alert-normal-item-value">
            <el-select
              v-model="testData.doctorList.default"
              clearable
              placeholder="请选择"
              size="mini"
              multiple
        ...
        >
          ...
          <div class="family-new-alert-normal-item-value">
            <div
              v-for="(item,index) in testData.doctorList.default"
              :key="index"
            >
              <div class="family-new-alert-normal-userhead-">
                <div class="family-new-alert-normal-userhead">
                  <img
                    :src="testData.doctorList.list.find(i=>i.value===item)?(testData.doctorList.list.find(i=>i.value===item).imgSrc || '../../static/assets/img/a-6.png'):'../../static/assets/img/a-6.png'"
                    alt
                  >
                  <i class="iconfont">&#xe611;</i>
                </div>
        ...
        <div
          class="family-new-alert-normal-item"
          v-if="testData.businessDescription.show"
        >
          <div class="family-new-alert-normal-item-name">
            <span>业务描述:</span>
          </div>
          ...
          <div class="family-new-alert-normal-item-value family-new-alert-normal-item-value-spe">
            <div class="input-item-value-div input-item-value-div-spe">
              <div class="family-new-alert-agreement-head">
                <input
                  type="text"
                  v-model="testData.agreement.default.label"
                  class="family-new-alert-edit-agreement"
                  @change="clearAgreementId"
                  autocomplete="off"
                >
                ...
                <Dropdown>
                  <p>
                    <Icon
                      type="ios-arrow-down"
                      class="agreement-arrow-down"
                    ></Icon>
                  </p>
                  <DropdownMenu slot="list">
                    <DropdownItem
                      v-for="(item,index) in testData.agreement.list"
                      :disabled="!testData.state"
                      :key="index"
                      @click.native="getAgreementSelect(item,index)"
              ...
              <div class="family-new-alert-agreement-content">
                <textarea
                  ...
                  rows="5"
                  :disabled="!testData.state"
                  v-model="testData.agreement.showContent"
                  class="family-new-alert-edit-agreement-content"
                  @change="clearAgreementId"
                ></textarea>
        ...
        <div
          class="family-new-alert-normal-item"
          v-if="testData.servicePhone.show"
        >
          <div class="family-new-alert-normal-item-name">
            <span>服务电话:</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    async getOrderFmsCharts() {
      ...
      if (res.data && res.data.errCode === 0) {
        this.statisticsInfo.count.dataAxis = res.data.body.data.map(
          item => item.x,
        )
    ...
    async getPushStatisticalData() {
      ...
      if (res.data && res.data.errCode === 0) {
        this.statisticsInfo.push.dataAxis = res.data.body.data.map(
          item => item.x,
        )
    ...
    async getQueryStatisticalData() {
      ...
      if (res.data && res.data.errCode === 0) {
        this.statisticsInfo.query.dataAxis = res.data.body.data.map(
          item => item.x,
        )
    ...
    async getFetchHospitalDepts() {
      ...
      if (res.data && res.data.errCode === 0) {
        ...
        this.statisticsInfo.departmentList.list = initArr.slice(
          0,
          initArr.length,
        )
        ...
        this.getSelectDepartment(
          this.testData.departmentList.default.value ||
            this.testData.departmentList.list[0].value,
        )
     ...
     */
    addBuss() {
      Promise.all([this.getToolBusinessType(), this.getStencilName()])
        .then(res => {
    ...
    async getSelectDepartment(data) {
      ...
      if (res.data && res.data.errCode === 0) {
        this.testData.doctorList.list = res.data.body.map(item => {
          ...
          item.imgSrc = item.headId
            ? `${apiBaseURL.developmentEnvironment}/m/v1/api/hdfs/fs/download/${
                item.headId
              }`
            : item.headId
          return item
    ...
    async getBussModuleList() {
      ...
      return
      let result = this.$store.state.user.userInfo.hasAuth.filter(
        item => item.type === '1',
      ) //筛选出管理权限
      let mapList = [
        //全部列表
    ...
    async saveInfo() {
      ...
      if (this.testData.businessTemplate.default.value === 'JTYS') {
        //家庭医生 这个模板和其他模板的价格是不一样的，判断下
        ...
        for (let i = 0; i < this.testData.businessPrice.data.length; i++) {
          ...
          if (item.worth.length < 1 || Number.isNaN(Number(item.worth)))
            continue
      ...
      postData[1].businessType =
        this.testData.businessTypeList.default.value === 'customize'
          ? this.testData.businessTypeList.default.label
          : this.testData.businessTypeList.default.value

      // return
      // const res = this.testData.type==='1'? await addBusiness(...postData):await updateBusiness(...postData);
      const res = this.testData.businessId
        ? await updateBusiness(...postData)
        : await addBusiness(...postData)
    ...
    async getChildrenBuss(id) {
      ...
      if (res.data && res.data.errCode === 0) {
        ...
        for (i; i < this.testData.businessPrice.data.length; i++) {
          ...
          this.testData.businessPrice.data[i].childList = JSON.parse(
    ...
    async setStatus(info) {
      this.showInfo.list.find(
        i => i.businessId === info.item.businessId,
      ).loading = true
      const res = await disableClinic(
        { token: this.userInfo.token },
        {
    ...
    async editItem(item) {
      ...
      item.price.forEach((v, k) => {
        priceList[k] = v
      }) */
      const priceList = deepCopy(item.price)
      // item.price
      const option = {
        show: false, //是否显示新增弹窗
        state: item.state,
